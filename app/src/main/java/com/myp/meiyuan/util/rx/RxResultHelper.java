package com.myp.meiyuan.util.rx;

import android.util.Log;

import com.myp.meiyuan.entity.BaseResult;
...
public class RxResultHelper {
                        ...
                        new Func1<BaseResult<T>, Observable<T>>() {
                            ...
                            public Observable<T> call(BaseResult<T> mDYResponse) {
                                ...
                                if (mDYResponse.surcess()) {
                                    ...
                                    return Observable.error(new RuntimeException(mDYResponse.getErrorMessage()));
                                }
                            }
                        }
    ...
    public static <T> Observable.Transformer<T, T> httpResult01() {
            ...
            public Observable<T> call(Observable<T> apiResponseObservable) {
                return apiResponseObservable.flatMap(
                        new Func1<T, Observable<T>>() {
                            @Override
                            public Observable<T> call(T mDYResponse) {
                                return createData(mDYResponse);
                            }
                        }
                ).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
            }
